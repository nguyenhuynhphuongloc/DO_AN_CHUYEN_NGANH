...
- Define a stable normalized receipt JSON contract that preserves parser output for review and downstream finance persistence, including merchant name, transaction date/time, total amount, currency, provider/parser category context, and line items.
...
- Treat parsed receipts as review drafts until confirmation:
  - receipt/parser JSON is persisted before finance transaction creation,
  - explicit confirmation creates the linked finance transaction and confirmed receipt linkage,
  - leaving the review flow keeps the draft receipt session available for later resume,
  - explicit discard or "do not save" removes the draft receipt/session without creating a confirmed receipt.
...
- `receipt-draft-lifecycle`: Preserve receipt parse drafts until confirmation, support explicit discard without receipt persistence, and link confirmed receipts to created transactions only after review.
...
- Affected receipt-session lifecycle logic in `microservices/receipt-service`: resumable draft sessions, explicit discard/no-save handling, and promotion of temporary receipt data only after confirmation.
