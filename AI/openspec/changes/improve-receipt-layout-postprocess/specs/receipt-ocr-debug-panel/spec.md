...
### Requirement: OCR debug payload SHALL expose layout refinement provenance
The system SHALL expose additive per-block layout refinement metadata so developers can inspect how coarse layout detections were transformed into receipt-aware semantic regions.

#### Scenario: A block is split during refinement
- **WHEN** the layout refinement stage splits a coarse source block into multiple semantic blocks
- **THEN** each emitted block SHALL include additive metadata for `original_label`, `refined_label`, `is_split`, and `split_source_block_id`

#### Scenario: A block is synthesized or demoted during refinement
- **WHEN** the layout refinement stage synthesizes an `items` block or demotes an artifact block to `metadata`
- **THEN** the emitted debug block SHALL include additive metadata indicating the refined label and whether the block was synthesized
