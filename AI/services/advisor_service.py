    ...
    try:
        ...
        messages = [
            {"role": "system", "content": (
                ...
                "Yêu cầu: Trả lời bằng tiếng Việt, súc tích, chuyên nghiệp, tập trung vào con số và đưa ra lời khuyên hữu ích. "
                "Nếu không có đủ dữ liệu để trả lời một câu hỏi cụ thể, hãy thành thật thông báo."
            )},
            {"role": "user", "content": f"Dữ liệu tài chính thực tế của tôi:\n{stats_txt}\n\nCâu hỏi: {query or 'Hãy nhận xét tình hình tài chính của tôi.'}"}
        ]
        ...
        return {
            "advice": f"Dữ liệu: Thu {context.get('totalIncome', 0):,.0f} | Chi {context.get('totalExpense', 0):,.0f} | Dư {context.get('balance', 0):,.0f} VND. "
                      f"Chatbot đang nạp bộ não AI (Qwen 3B), vui lòng đợi một chút."
        }
