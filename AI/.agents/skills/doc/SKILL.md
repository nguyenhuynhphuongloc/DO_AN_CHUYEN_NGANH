...
2. Use `python-docx` for edits and structured creation (headings, styles, tables, lists).
   - For repo-local document generation, prefer `scripts/create_doc.py` when the input can be expressed as structured JSON with a title and sections.
...
Structured DOCX creation:
```
python3 scripts/create_doc.py --input-json /path/to/content.json
```

Example JSON payload:
```json
{
  "title": "OCR System Overview",
  "sections": [
    {
      "heading": "Introduction",
      "paragraphs": [
        "This thesis chapter introduces the OCR system and its role in the project."
      ]
    }
  ]
}
