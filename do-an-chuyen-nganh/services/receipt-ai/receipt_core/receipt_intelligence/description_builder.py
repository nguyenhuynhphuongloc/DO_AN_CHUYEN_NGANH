...
def build_receipt_description(
    *,
    ...
    if date_label:
        sentence_parts.append(f"Ngày {date_label}")
    if merchant:
        sentence_parts.append(f"chi tại {merchant}")
    elif category:
        sentence_parts.append(f"chi cho {category}")
    if amount_label:
        sentence_parts.append(f"số tiền {amount_label}")
    if category and merchant:
        sentence_parts.append(f"thuộc nhóm {category}")
