...
The system SHALL provide a protected OCR page inside the finance application shell that lets an authenticated user upload a receipt image, preview the image, trigger OCR, observe loading or progress states, review important extracted fields, edit the final transaction values, and confirm save into finance records.

#### Scenario: Authenticated user uploads a receipt on the OCR page
- **WHEN** an authenticated user selects a receipt and starts OCR
- **THEN** the frontend calls the configured OCR endpoint and shows the upload, preview, loading, success, or error state on the protected OCR page

#### Scenario: User edits important fields before save
- **WHEN** OCR returns important extracted invoice fields
- **THEN** the OCR page shows editable values for merchant name, transaction date, total amount, tax amount, payment method, suggested category, wallet, final category, note, and relevant line-item details before save

### Requirement: The OCR page uses finance data to drive wallet and category confirmation
The system SHALL load the authenticated user’s wallets and categories from `finance-service`, SHALL prefill category selection by matching the AI-suggested category when possible, and SHALL allow the user to override the final wallet and category values during review.
