...
The system SHALL allow a host application to configure the OCR request target, `auth-service` target, and `finance-service` save target through frontend configuration without changing the component or page internal implementation.
...
- **THEN** the frontend sends auth, OCR, wallet/category loading, and save requests to those configured endpoints
...
The system SHALL show distinct loading and error states for auth, OCR, finance-data loading, and transaction save operations, SHALL surface backend error messages to the user, and SHALL show a success message after a confirmed OCR transaction is created.
